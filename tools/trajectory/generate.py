...
def generate_server_data(agents=100, seed=1, minutes=10):
...
def generate_client_data(client_size, seed=1, minutes=10):
    agents = 1
    state = np.random.RandomState(seed)
...
if __name__ == '__main__':
    ...
    if args.target == "server":
        generate_server_data(agents=args.size, seed=seed, minutes=minutes)
    elif args.target == "client":
        generate_client_data(client_size=args.size, seed=seed, minutes=minutes)
    else:
